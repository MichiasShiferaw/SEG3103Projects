...
/**
 * A class representing a Boat’s mobility implementation. This class has common traits of a Boat such as its current state represented by an enum (a special collection) value as well as another
 *  enum to indicate whether the Boat's engine is running or not. The Boat machine also has a plethora of behaviors like by pressing the left pedal once, it will accelerate forward or press both
 *  pedals simultaneously, the Boat will brake). Each method below represents the behaviour of a Boat.
...
public class Boat{
    ...
    //Boat Functional Attributes
    /**Initialize an enum collection that consists of the state a Boat can have*/
    public enum State {
        /**State of when the engine is off*/
        engineOff,
        /** State of when the Boat is on*/
        vehiculeOn}
    /**Initialize an enum collection that consists of the state a Boat can have turn the engine is running*/
    ...
    private String msg;
    private Boat.StateVehiculeOn prev1 = stateVehiculeOn; //Initialize a StateVehiculeOn variable that holds the previous state of the Boat object

    /**This method CREATE a NEW Boat objects that is set to engine off*/
    public Boat(){
     ...
     * @param state1 the current state of the Boat, used to vocalize that the state remains unchanged
     * @return a String that announces the Boat will remain at the current state */
    private String noChange(StateVehiculeOn state1){
        return (" the buggy will remain at "+ stateVehiculeOn) +".";}
    /** This method is used to show that the change of state when the Boat is on was unsuccessful
     * @param state1 the current state of the Boat, used to vocalize that the state remains unchanged
     * @return a String that announces the Boat will remain at the current state */
    //private String noChange(State state1) {return (" the buggy will remain at still, on the" + state1) + " state.";}

    /** This method is getter that converts the state of the Boat into an output string
    ...
    /** This method is a getter for the current state of the Boat
     * @return state, the current state of the Boat (i.e. VehiculeOn or engineOff) */
    public State getState(){return state;}

    /** This method is a getter for the current state of the Boat when turned on
     * @return stateVehiculeOn, the current state of the Boat when turned on (i.e. turning or accelerating forward) */
    ...
    /**This method portrays when the LEFT PEDAL is tapped TWICE in hopes to change the state of the Boat
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /** This method REMOVES THE KEY from the key spot in hopes to turn the Boat engine off
     @return a Boolean that expressed if the Boat was successfully turned off*/
    ...
    /**This method portrays when the RIGHT PEDAL is tapped in hopes to change the state of the Boat
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /**This method portrays when the LEFT PEDAL is tapped ONCE in hopes to change the state of the Boat
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /**This method portrays when the LEFT PEDAL (only) is tapped and depending of the length (>=5 seconds) the pressure on the pedal determine if the state of the Boat will change
     * @return a Boolean that expresses if the transfer of state was processed*/
    public boolean pressLeftPedal(){
        ...
        switch (aStateVehiculeOn){case neutral:
            ...
            if (timer>=5){setStateVehiculeOn(StateVehiculeOn.acceleratebackwards);exitStateVehiculeOn(); wasEventProcessed = true;break;} else {msg += (" since you held it for " + timer + " seconds, the Boat will remain at its current state");}break;
            //default: msg+=noChange(stateVehiculeOn);
        }
        System.out.println(msg);
        return wasEventProcessed;  }
    ...
    /**This method portrays when the RIGHT PEDAL is tapped TWICE in hopes to change the state of the Boat
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /**This method portrays when the RIGHT PEDAL is tapped ONCE and depending of the length (>=5 seconds) the pressure on the pedal determine if the state of the Boat will change
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /**This method portrays when the FIRST BUTTON is pressed while pressure is applied on BOTH PEDALS and in hopes to change the state of the Boat
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /**This method portrays when the Boat releases the pressure from the RIGHT PEDAL (only) and in hopes to change the state of the Boat
     * @return a Boolean that expresses if the transfer of state was processed*/
    ...
    /** This method portrays when the Boat RELEASE the pressure from BOTH PEDALS and changes the state of the Boat
    ...
    /** This method an elementary step when changing gears/state of the Boat. If the Boat is moving, this method will call another method to help change gears, otherwise the Boat remains turned off*/
    //private void exitState(){switch(state){case vehiculeOn:exitStateVehiculeOn();break;case engineOff:break;}}

    /** This method is called to set the state of the Boat, based on if it is parked or not.
    ...
    /**This method verifies if the state provided in the parameter then sets the state of the Boat object to the parameter provided
     * @param aStateVehiculeOn An StateVeichuleOn value represent the desired new state of the Boat
     * @return*/
